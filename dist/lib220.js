    ...
    function loadURLHandler(defaultOutput, loadFunction) {
        return function (url) {
            ...
            return runner.pauseImmediate(() => {
                ...
                const encodedURL = encodeURIComponent(url), baseUrl = config.url, getUrlLink = `${baseUrl}geturl?`, queryURL = `${getUrlLink}url=${encodedURL}&user=${userEmail}&session=${sessionId}`;
